...
// function ChartStyle({ config }: { config?: Record<string, ChartConfig> }) {
...
//   if (key && payload && key in payload && typeof payload[key as keyof typeof payload] === "string") {
//     configLabelKey = payload[key as keyof typeof payload] as string
//   } else if (
//     payloadPayload &&
//     key &&
...
//   // Add null check for configLabelKey
//   if (!configLabelKey || !config) return undefined
...
function ChartStyle({ config }: { config?: Record<string, any> }) {
  // Add null check before using Object.entries
  if (!config) return null
  ...
  // Use standard style element instead of styled-jsx
  return <style dangerouslySetInnerHTML={{ __html: `:root { ${cssString} }` }} />
}
