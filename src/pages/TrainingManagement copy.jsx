...
const TrainingManagement = () => {
  ...
  const handleAssignUser = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/training-groups/${selectedGroup}/assign`, { userId: selectedUser }, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
      ...
      <div className="mb-8">
        ...
        <form onSubmit={handleCreateGroup} className="space-y-2">
          ...
          <select
            ...
            onChange={(e) => setNewGroup({ ...newGroup, period: e.target.value })}
            className="border p-2 rounded"
          >
          ...
          <select
            ...
            onChange={(e) => setNewGroup({ ...newGroup, trainingCenter: e.target.value })}
            className="border p-2 rounded"
          >
            ...
            {trainingCenters?.length > 0 && Array.isArray(trainingCenters) && trainingCenters.map((center) => (
              <option key={center._id} value={center._id}>
                {center.trainingCentreName}
              </option>
            ))}
      ...
      <div className="mb-8">
        ...
        <div className="space-y-2">
          ...
          >
            ...
            {users?.length > 0 && Array.isArray(users) && users.map((user) => (
              <option key={user._id} value={user._id}>
                {user?.firstName} {user?.lastName}
              </option>
            ))}
          </select>
          <button onClick={handleAssignUser} className="bg-purple-500 text-white px-4 py-2 rounded">
      ...
      <ul>
        {trainingGroups.map((group) => (
          <li key={group._id} className="mb-4">
            ...
            <p>Period: {group.period ? `${group.period.name} - ${group.period.year}` : 'N/A'}</p>
        ...
        <div className="mb-8">
          ...
          <ul>
            {trainingGroups.map((group) => (
              <li key={group._id} className="mb-4">
                <h3 className="font-semibold">{group.name}</h3>
                <p>Period: {group.period?.name} - {group.period?.year}</p>
                <p>Training Center: {group.trainingCenter ? group.trainingCenter.trainingCentreName : 'N/A'}</p>
                <p>Start: {new Date(group.startTime).toLocaleString()}</p>
                <p>End: {new Date(group.endTime).toLocaleString()}</p>
                <h4 className="font-semibold mt-2">Users:</h4>
                <ul>
