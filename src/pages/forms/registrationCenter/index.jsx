...
const checkAsssessment = ({ form }) => {
  ...
  if (assessment.trainingCurriculum === "yes" && !assessment.curriculumAttachment) {
    errorMsg = "Please attach the curriculum to proceed!"
  }
...
const checkLegal = ({ form }) => {
  try {
    ...
    if (!legalInfo.legalRegistration || legalInfo.legalRegistration.trim() === "") {
    ...
    console.log("Trade Areas:", tradeAreas);
    ...
    for (let i = 0; i < tradeAreas.length; i++) {
      ...
      if (isNaN(instructors) || instructors <= 0) {
        return { erroMsg: `Trade Area #${i+1}: Number of instructors must be greater than 0!` };
      }
      
      // Convert to number and check if valid
      const trainees = Number(area.trainees);
      if (isNaN(trainees) || trainees <= 0) {
        return { erroMsg: `Trade Area #${i+1}: Number of trainees must be greater than 0!` };
      }
...
const checkValidateBank = ({ form }) => {
  ...
  if (!bankAccount.accountName || !bankAccount.accountNumber || !bankAccount.bank) {
    erroMsg = "All bank details are required!"
  }
...
const TrainingCenterForm = () => {
  ...
  const goNext = () => {
    try {
      ...
      toast.error("An error occurred while validating the form", { position: "top-right" });
    }
  }
