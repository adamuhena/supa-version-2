...
const MultiSelect = ({ options, selected, onChange, placeholder }) => {
  ...
  const filteredOptions = options.filter((option) => {
    const label = typeof option.label === 'string' ? option.label : option.label.props.children.join(' ');
  ...
  const renderLabel = (label) => {
    if (typeof label === 'string') {
      ...
      return label;
    } else if (typeof label === 'object' && label.props && label.props.children) {
      ...
      <Command className="overflow-visible bg-transparent">
        ...
        {open && (
          <div className="absolute mt-2 w-full z-50">
            <CommandPrimitive.List className="max-h-[300px] overflow-auto rounded-md border bg-popover text-popover-foreground shadow-md animate-in">
              {filteredOptions.length === 0 && (
                <CommandEmpty>No results found.</CommandEmpty>
              )}
              <CommandGroup>
