...
const UserManagement = () => {
  ...
  const fetchUsers = useCallback(async (page = 1, limit = 50, role = "", searchQuery = "") => {
    setLoading(true)
    try {
      const accessToken = localStorage.getItem("accessToken")
      const response = await axios.get(`${API_BASE_URL}/usersmgt`, {
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setUsers((prevUsers) => prevUsers.map((user) => (user._id === editUser._id ? response.data.data : user)))
        toast.success("User Updated Successfully")
        setIsEditDialogOpen(false)
        setNewPassword("")
      }
    ...
    <ProtectedRoute>
      ...
      <div className="container mx-auto p-6">
        <header className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold">USER MANAGEMENT</h1>
            <h2 className="text-left font-[700] text-[14px]">(USERS & ADMINISTRATORS)</h2>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => navigate("/biodata")}>
              <UserCircle className="mr-2 h-4 w-4" /> Update Profile
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Edit User</DialogTitle>
            <DialogDescription>Update user details or change password. Click save when you're done.</DialogDescription>
          </DialogHeader>
          {editUser && (
            <form onSubmit={handleUpdateUser} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
