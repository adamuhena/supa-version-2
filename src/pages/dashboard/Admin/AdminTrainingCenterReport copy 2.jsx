...
const TrainingCenterReport = () => {
    ...
    try {
      ...
      const formatted = formatTCToCSV(
        (data || []).map((x, i) => {
          ...
          (x?.legalInfo?.tradeAreas || []).forEach((area) => {
            // Handle case where area has sector objects (second payload format)
            if (area?.sector && area.sector.length > 0) {
              area.sector.forEach((sector) => {
                if (sector?.name) {
                  sectors += sector.name + ", ";
                  
                  (area?.tradeArea || []).forEach((ta_id) => {
            ...
            // Handle case where tradeArea is an array of strings (first payload format)
            else if (area?.tradeArea && area.tradeArea.length > 0) {
              area.tradeArea.forEach((tradeAreaName) => {
                if (tradeAreaName && tradeAreaName.trim()) {
                  tradeAreas += tradeAreaName + ", ";
                }
              });
            }
          });
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      // Process the data to ensure consistent display of trade areas
      const processedData = data.map(center => {
        // Ensure legalInfo exists
        if (!center.legalInfo) {
          center.legalInfo = { tradeAreas: [] };
        }
        
        // Ensure tradeAreas exists
        if (!center.legalInfo.tradeAreas) {
          center.legalInfo.tradeAreas = [];
        }
                            ...
                            <AlertDialogDescription>
                              {(center?.legalInfo?.tradeAreas || []).map(
                                (area, index) => {
                                  ...
                                  if (area?.sector && area.sector.length > 0) {
                                    return (
                                      <div key={area._id || index} className="mb-[30px]">
                                        {(area?.sector || []).map((sector) => (
                                          <div key={sector._id} className="mb-[30px]">
                                            ...
                                            <div className="flex gap-[15px] flex-wrap">
                                              {(area?.tradeArea || []).map((ta_id) => {
                                                ...
                                                return found ? (
                                                  <p key={found._id} className="flex items-center gap-1">
                                  ...
                                  // Handle case where tradeArea is an array of strings (first payload format)
                                    ...
                                    return (
                                      <div key={area._id || index} className="mb-[30px]">
                                        ...
                                        <div className="flex gap-[15px] flex-wrap">
                                          {area.tradeArea.map((tradeAreaName, idx) => (
                                            tradeAreaName && tradeAreaName.trim() ? (
                                              <p key={idx} className="flex items-center gap-1">
                                                <div className="w-2 h-2 bg-black rounded-full" />
                                                <span>{tradeAreaName}</span>
                                              </p>
                                            ) : null
                                          ))}
                                        </div>
